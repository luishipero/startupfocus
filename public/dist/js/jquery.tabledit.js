    ...
    $.fn.Tabledit = function(options) {
                    ...
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                var input = '<input class="tabledit-input requiredField' + settings.inputClass + '" type="number" name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none; max-width: 120px;" disabled required/>';
                            }
        ...
        var Edit = {
            ...
            submit: function(td) {
                ...
                // ajax('edit');
                $.ajax({
                    url: '/api/tables/' + $(td).parents('tr').find('row').prevObject.attr('id') + "?" +
                $('form.tabledit-form').serialize(),
                    type: 'POST',
        ...
        var Remove = {
            ...
            submit: function(button) {
                ...
                // ajax('remove');
                $.ajax({
                    url: '/api/expenses/' + $(button).parents('tr').find('row').prevObject.attr('id'),
                    type: 'DELETE',
                    success: function(result) {
                        // Do something with the result
                    }
        ...
        function ajax(action)
        {
            ...
            // console.log("HERE CONES ID" + $(this).parent('tr').attr('id'));
